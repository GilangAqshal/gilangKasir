...
if (!empty($_SESSION['petugas'])) {
    ...
    if (!empty($_GET['pengaturan'])) {
        ...
        $row -> execute($data);
        echo '<script>window.location="../../petugas.php?page=pengaturan&success=edit-data"</script>';
    }
    ...
    if (!empty($_GET['kategori'])) {
        ...
        $row = $config -> prepare($sql);
        $row -> execute($data);
        echo '<script>window.location="../../petugas.php?page=kategori&uid='.$id.'&success-edit=edit-data"</script>';
    }
    ...
    if (!empty($_GET['stok'])) {
        ...
        $row = $config -> prepare($sql);
        $row -> execute($data);
        echo '<script>window.location="../../petugas.php?page=barang&success-stok=stok-data"</script>';
    }
    ...
    if (!empty($_GET['barang'])) {
        ...
        $row = $config -> prepare($sql);
        $row -> execute($data);
        echo '<script>window.location="../../petugas.php?page=barang/edit&barang='.$id.'&success=edit-data"</script>';
    }
    ...
    if (!empty($_GET['gambar'])) {
        ...
        if(!in_array($filetype, array_keys($allowedTypes))) {
            echo '<script>alert("You can only upload JPG, PNG and GIF file");window.location="../../petugas.php?page=user"</script>';
            exit;
            ...
            // 		<a href='../../petugas.php?page=user'>Back to upform</a><BR>";
            echo '<script>alert("You can only upload JPG, PNG and GIF file");window.location="../../petugas.php?page=user"</script>';
            exit;
            ...
            // echo "<font face='Verdana' size='2' ><BR><BR><BR>
            // 		<a href='../../petugas.php?page=user'>Back to upform</a><BR>";
            echo '<script>alert("WARNING !!! Besar Gambar Tidak Boleh Lebih Dari 4 MB");window.location="../../petugas.php?page=user"</script>';
            exit;
        } else {
            $dir = '../../assets/img/user/';
            ...
            if (move_uploaded_file($tmp_name, $dir.$name)) {
                ...
                $row = $config -> prepare($sql);
                $row -> execute($data);
                echo '<script>window.location="../../petugas.php?page=user&success=edit-data"</script>';
            } else {
                echo '<script>alert("Masukan Gambar !");window.location="../../petugas.php?page=user"</script>';
                exit;
            }
        }
    }
    ...
    if (!empty($_GET['jual'])) {
        ...
        if ($hasil['stok'] > $jumlah) {
            ...
            $row1 -> execute($data1);
            echo '<script>window.location="../../petugas.php?page=jual#keranjang"</script>';
        } else {
            echo '<script>alert("Keranjang Melebihi Stok Barang Anda !");
					window.location="../../petugas.php?page=jual#keranjang"</script>';
        }
    }
    ...
    if (!empty($_GET['cari_barang'])) {
		...
		<?php foreach ($hasil1 as $hasil) {?>
			<tr>
				...
				<td>
				<a href="petugasfungsi/tambah/tambah.php?jual=jual&id=<?php echo $hasil['id_barang'];?>&id_kasir=<?php echo $_SESSION['petugas']['id_member'];?>" 
