...
if (!empty($_SESSION['petugas'])) {
    ...
    if (!empty(htmlentities($_GET['kategori']))) {
        ...
        $row -> execute($data);
        echo '<script>window.location="../../petugas.php?page=kategori&&remove=hapus-data"</script>';
    }
    ...
    if (!empty(htmlentities($_GET['barang']))) {
        ...
        $row = $config -> prepare($sql);
        $row -> execute($data);
        echo '<script>window.location="../../petugas.php?page=barang&&remove=hapus-data"</script>';
    }
    ...
    if (!empty(htmlentities($_GET['jual']))) {
        ...
        $row = $config -> prepare($sql);
        $row -> execute($data);
        echo '<script>window.location="../../petugas.php?page=jual"</script>';
    }
    ...
    if (!empty(htmlentities($_GET['laporan']))) {
        ...
        $row = $config -> prepare($sql);
        $row -> execute();
        echo '<script>window.location="../../petugas.php?page=laporan&remove=hapus"</script>';
    }
}
