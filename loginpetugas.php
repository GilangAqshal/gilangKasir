<?php
    ...
    if(isset($_POST['proses'])){
        ...
        $sql = 'SELECT member.*, login.user, login.pass, login.role
                ...
                WHERE user = ? AND pass = MD5(?) AND role = "petugas"'; // Hanya mengambil role "petugas"
        $row = $config->prepare($sql);
        $row->execute(array($user, $pass));
        $jum = $row->rowCount();
        if($jum > 0){
            $hasil = $row->fetch();
            $_SESSION['petugas'] = $hasil; // Menyimpan data petugas ke dalam session
                ...
                <div class="card o-hidden border-0 shadow-lg my-5">
                    <div class="card-body p-0">
						...
						<div class="p-5">
							...
							<hr>
                            <div class="text-center">
								<a class="small" href="login.php">Login as Admin</a>
							</div>
						</div>
                    </div>
                </div>
